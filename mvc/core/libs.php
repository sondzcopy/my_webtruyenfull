<?php  require_once "./mvc/core/route.php";  require_once "./mvc/models/dbCon.php";
	...
	function DateToTime($date){
        ...
        function getTheLoai_Header(){
           ...
           try{
                $query = "SELECT * FROM truyen_theloai ORDER BY ten ASC";
                $cmd = $conn->prepare($query);
                $cmd->execute();
        ...
        function getTheLoai_HOME($truyen_id){
                ...
                $conn = $conn->connect();
                   ...
                   try{
                        ...
                        foreach($theloai as $val){
                            if($i < count($theloai))
                                $result .= '<a itemprop="genre" class="hover-title text-decoration-none text-dark category-name" href="#">'.$val['ten'].'</a>, ';
                            else
                               $result .= '<a itemprop="genre"  class="hover-title text-decoration-none text-dark category-name" href="#">'.$val['ten'].'</a>';
                           $i++;
                        }
        ...
        function disable($truyen_id, $soChuong){
            if(checkChuongSau($truyen_id, $soChuong) == false){
                echo 'disabled';
            }
        }
?>